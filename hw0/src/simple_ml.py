...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, m, batch):
        ...
        X_ = X[i:i + batch]
        y_ = y[i:i + batch]
        Z = np.exp(X_ @ theta) / (np.exp(X_ @ theta).sum(axis=1)[:, np.newaxis])

        I_y = np.eye(k)[y_]
      
        grad = X_.T @ (Z - I_y) / len(y_)
        theta[:] = theta - lr * grad
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, m, batch):
        ...
        Z1 = np.maximum(0, X_ @ W1)
        Z2 = Z1 @ W2

        G2 = np.exp(Z2) / np.exp(Z2).sum(axis=1)[:,np.newaxis] - I_y
        G1 = np.float32(Z1>0) * (G2 @ W2.T)
